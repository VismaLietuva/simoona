(function () {
    ...
    angular.module('simoonaApp.Map').directive('mapDir', mapDir);
    ...
    function mapDir(
        ...
        var directive = {
            ...
            link: function (scope, element, attrs) {
                ...
                scope.$watch('params.highlightedRoomId', function (newId) {
                    if (!newId) {
                        return;
                    }
                    ...
                    $timeout(function () {
                        ...
                        if (!zoom.isCoordsVisible(center.x, center.y)) {
                            var diff = zoom.getCoordsVisiblilityDiff(
                                center.x,
                                center.y
                            );
                ...
                function changeBackground() {
                    ...
                    if (
                        picture.width < paperSize.width &&
                        picture.height < paperSize.height
                    ) {
                        //scope.aspectRatio = 1;
                        ...
                        scope.aspectRatio =
                            picture.width > paperSize.width
                                ? paperSize.width / picture.width
                                : picture.width / paperSize.width;
                        if (
                            paperSize.height <
                            picture.height * scope.aspectRatio
                        ) {
                            scope.aspectRatio =
                                paperSize.height / picture.height;
                        }
                    } else {
                        scope.aspectRatio =
                            picture.height > paperSize.height
                        ...
                        if (
                            paperSize.width <
                            picture.width * scope.aspectRatio
                        ) {
                            scope.aspectRatio = paperSize.width / picture.width;
                        }
                    }
                    ...
                    scope.background = scope.paper
                        .image(
                            $filter('picture')(scope.floor.pictureId),
                            0,
                            0,
                ...
                function drawRoom() {
                    if (
                        $state.current.name ===
                        'Root.WithOrg.Admin.Offices.Floors.Rooms.List'
                    ) {
                        ...
                        scope.svg.bind('mousedown', function (event) {
                            if (event.which === 1) {
                                ...
                                var state = roomManageService.checkPoints(
                                    coords,
                                    scope
                                );
                                if (state) {
                                    ...
                                    element.bind('mousemove', function (event) {
                                        ...
                                        roomManageService.drawTempLine(
                                            currentCoords
                                        );
                                    });
                                }
