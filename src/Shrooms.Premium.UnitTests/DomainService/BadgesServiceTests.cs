using System;
using System.Collections.Generic;
using System.Data.Entity;
using System.Threading.Tasks;
using NSubstitute;
using NUnit.Framework;
using Shrooms.Constants.BusinessLayer;
using Shrooms.DataLayer.DAL;
using Shrooms.Domain.Services.Badges;
using Shrooms.EntityModels.Models;
using Shrooms.EntityModels.Models.Badges;
using Shrooms.EntityModels.Models.Kudos;
using Shrooms.UnitTests.Extensions;

namespace Shrooms.Premium.UnitTests.DomainService
{
    class BadgesServiceTests
    {
        private BadgesService _badgesService;

        private IUnitOfWork2 _unitOfWork;

        private IDbSet<BadgeCategory> _badgeCategoriesDbSet;
        private IDbSet<BadgeLog> _badgeLogsDbSet;
        private IDbSet<KudosLog> _kudosLogsDbSet;

        [SetUp]
        public void TestInitializer()
        {
            _unitOfWork = Substitute.For<IUnitOfWork2>();

            _badgeCategoriesDbSet = _unitOfWork.MockDbSet<BadgeCategory>();
            _badgeLogsDbSet = _unitOfWork.MockDbSet<BadgeLog>();
            _kudosLogsDbSet = _unitOfWork.MockDbSet<KudosLog>();

            _badgesService = new BadgesService(_unitOfWork);
        }

        [Test]
        public async Task Should_Assign_1_Badge_For_Recent_Kudos_Transaction()
        {
            // Arrange
            var twoDaysAgo = DateTime.UtcNow.AddDays(-2);
            var oneHourAgo = DateTime.UtcNow.AddHours(-1);

            var badgeCategories = new List<BadgeCategory>
            {
                new BadgeCategory
                {
                    Id = 1,
                    Created = twoDaysAgo,
                    Description = "Choco desc",
                    Title = "Choco",
                    BadgeTypes = new List<BadgeType>
                    {
                        new BadgeType { Id = 1, BadgeCategoryId = 1, Value = 15, Created = twoDaysAgo, Title = "Cookie", IsActive = true },
                        new BadgeType { Id = 1, BadgeCategoryId = 1, Value = 16, Created = twoDaysAgo, Title = "Cookie monster", IsActive = true }
                    },
                    RelationshipsWithKudosTypes = new List<BadgeCategoryKudosType>
                    {
                        new BadgeCategoryKudosType
                        {
                            KudosType = new KudosType { Name = "Choco" },
                            CalculationPolicyType = BadgeCalculationPolicyType.PointsStrategy
                        },
                        new BadgeCategoryKudosType
                        {
                            KudosType = new KudosType { Name = "Choco two kudos type" },
                            CalculationPolicyType = BadgeCalculationPolicyType.MultiplierStrategy
                        }
                    }
                },
                new BadgeCategory
                {
                    Id = 2,
                    Created = twoDaysAgo,
                    Description = "Ment desc",
                    Title = "Ment",
                    BadgeTypes = new List<BadgeType>
                    {
                        new BadgeType { BadgeCategoryId = 2, Created = twoDaysAgo, Title = "Rookie mentor", IsActive = true }
                    },
                    RelationshipsWithKudosTypes = new List<BadgeCategoryKudosType>
                    {
                        new BadgeCategoryKudosType
                        {
                            KudosType = new KudosType { Name = "Ment" },
                            CalculationPolicyType = BadgeCalculationPolicyType.MultiplierStrategy
                        }
                    }
                }
            };

            var kudosLogs = new List<KudosLog>
            {
                new KudosLog
                {
                    EmployeeId = "user1",
                    Employee = new ApplicationUser
                    {
                        Id = "user1", OrganizationId = 1, EmploymentDate = twoDaysAgo, BadgeLogs = new List<BadgeLog>()
                    },
                    Points = 10,
                    MultiplyBy = 2,
                    KudosTypeName = "Choco",
                    Status = KudosStatus.Approved,
                    Modified = oneHourAgo,
                    KudosSystemType = ConstBusinessLayer.KudosTypeEnum.Ordinary
                },
                new KudosLog
                {
                    EmployeeId = "user1",
                    Employee = new ApplicationUser
                    {
                        Id = "user1", OrganizationId = 1, EmploymentDate = twoDaysAgo, BadgeLogs = new List<BadgeLog>()
                    },
                    Points = 6,
                    MultiplyBy = 5,
                    KudosTypeName = "Choco two kudos type",
                    Status = KudosStatus.Approved,
                    Modified = oneHourAgo,
                    KudosSystemType = ConstBusinessLayer.KudosTypeEnum.Ordinary
                }
            };

            _badgeCategoriesDbSet.SetDbSetDataForAsync(badgeCategories);
            _kudosLogsDbSet.SetDbSetDataForAsync(kudosLogs);
            _badgeLogsDbSet.SetDbSetDataForAsync(new List<BadgeLog>());

            _unitOfWork.SaveChangesAsync().Returns(Task.FromResult(1));

            // Act
            await _badgesService.AssignBadgesAsync();

            // Assert
            _badgeLogsDbSet.Received().Add(Arg.Is<BadgeLog>(x => x.EmployeeId == "user1" && x.BadgeTypeId == 1 && x.OrganizationId == 1));
            Received.InOrder(async () => await _unitOfWork.SaveChangesAsync());
        }

        [Test]
        public void Should_Assign_1_Badge_For_Recent_New_Badge_Category()
        {
            // Arrange

            // Act

            // Assert

        }

        [Test]
        public void Should_Assign_1_Badge_For_Recent_New_Badge_Type()
        {
            // Arrange

            // Act

            // Assert

        }
    }
}
