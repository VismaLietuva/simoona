...
namespace Shrooms.API.Controllers.Wall
{
    ...
    public class WallController : BaseController
    {
        ...
        [Route("AllPosts")]
        [PermissionAuthorize(Permission = BasicPermissions.Post)]
        public async Task<IHttpActionResult> GetAllPagedWall(int page = 1, int wallPreference = 1) // wallType = 1 "My walls"   wallType = 2 "All walls"
        {
            var userAndOrg = GetUserAndOrganization();
            var wallPosts = await _wallService.GetAllPosts(page, ConstWebApi.DefaultPageSize, userAndOrg, wallPreference);
