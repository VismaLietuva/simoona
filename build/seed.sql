-- =============================================================================
-- Simoona – Reference Data Seed
-- Safe to run on any database (fresh or existing) – all inserts are guarded
-- with IF NOT EXISTS.  Run this after EF Core migrations have been applied.
--
-- Usage (from host):
--   docker exec simoona-db /opt/mssql-tools18/bin/sqlcmd \
--     -S localhost -U sa -P 'Password!123' -d SimoonaDB -b -No \
--     -i /seed.sql
-- Or use the setup.ps1 / setup.sh wrapper scripts.
-- =============================================================================
...
-- ---------------------------------------------------------------------------
-- Organizations  (must come before AspNetRoles / Walls)
-- ---------------------------------------------------------------------------
IF NOT EXISTS (SELECT 1 FROM dbo.Organizations WHERE Id = 1)
BEGIN
    SET IDENTITY_INSERT dbo.Organizations ON
    INSERT dbo.Organizations
        ([Id],[Name],[ShortName],[Created],[CreatedBy],[Modified],[ModifiedBy],[IsDeleted],
         [HostName],[HasRestrictedAccess],[WelcomeEmail],[RequiresUserConfirmation],[CalendarId],
         [TimeZone],[BookAppAuthorizationGuid],[CultureCode],[AuthenticationProviders],[KudosYearlyMultipliers])
    VALUES
        (1, N'testorg', N'testorg', '1900-01-01', NULL, '1900-01-01', NULL, 0, NULL, 0,
         N'<p style="text-align:center; font-size:14px; font-weight:400; margin: 0 0 0 0;">Administrator has confirmed your registration</p>',
         0, NULL, N'FLE Standard Time', NULL, N'en-US', N'internal;google;facebook', NULL)
    SET IDENTITY_INSERT dbo.Organizations OFF
END

-- ---------------------------------------------------------------------------
-- AspNetRoles  (FK → Organizations)
-- ---------------------------------------------------------------------------
